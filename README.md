...
In GBBS Pro, the message database files were used for two purposes: bulletins (a.k.a. boards, news groups, message bases, etc.) and email. While the format is the same, they have slightly different usage patterns (see technical documentation for details). The key difference with the email pattern is that the recipient of a message is not part of the message content, rather it uses the directory entry offset as a way to map to a user ID in the BBS. As such, a feature for email extraction allows for the use of specifying a GBBS Pro `USERS` file to determine the identity of the recipient.
...
If there is something you need fixed, added, or whatever, raise a ticket on the github page or email me at brian-removethispart *at* dronefone.com . You can also find information about this on my website at [http://dronefone.com/gbbs](http://dronefone.com/gbbs)
